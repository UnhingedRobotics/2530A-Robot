/[connor@laptop 2530A-Robot-Skills]$ cat src/main.cpp
/*----------------------------------------------------------------------------*/
/*                                                                            */
/*    Module:       main.cpp                                                  */
/*    Author:       Admin                                                     */
...
void autonomous(void) {
  ...
  goalclamp.set(false);
	intakeon = true;
	vex::thread intakeTast(intakeauton);
	PID_dist(-15, 0.5, 0.09, 0, 5);
  wait(0.5, seconds);
  goalclamp.set(true);
  wait(0.2, seconds);
	PID_theta(90, 0.15, 0.11, 0.04, 5);
  wait(0.2, seconds);
	PID_dist(25, 0.37, 0.09, 0, 5);
  wait(0.2, seconds);
  // leftdrive.spinFor(forward, 80, vex::rotationUnits::deg, false);
	PID_theta(180, 0.7, 0.11, 0.04, 5);
  wait(0.2, seconds);
	PID_dist(-35, 1.1, 0.09, 0, 5);
  intakeon = false;
	// intakeon = false;
